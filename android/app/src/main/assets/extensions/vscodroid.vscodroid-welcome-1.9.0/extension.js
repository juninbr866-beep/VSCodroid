...
const MESSAGES = {
    en: {
        noWorkspace: 'Open a folder before creating a project.',
        promptTitle: 'Create a project',
        placeholder: 'my-project',
        invalidName: 'Use letters, numbers, dots, dashes, and underscores.',
        exists: 'A folder with this name already exists.',
        created: 'Project created: {0}',
    },
    pt: {
        noWorkspace: 'Abra uma pasta antes de criar um projeto.',
        promptTitle: 'Criar um projeto',
        placeholder: 'meu-projeto',
        invalidName: 'Use letras, números, pontos, hífens e sublinhados.',
        exists: 'Já existe uma pasta com este nome.',
        created: 'Projeto criado: {0}',
    },
};
const message = (key) => {
    const language = String(vscode.env?.language || 'en').toLowerCase();
    const bundle = language === 'pt-br' || language === 'pt' ? MESSAGES.pt : MESSAGES.en;
    return bundle[key];
};
...
function copyTemplate(templateId, destination, projectName) {
    const source = path.join(__dirname, 'templates', templateId);
    fs.mkdirSync(destination, { recursive: true });
    for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
        const from = path.join(source, entry.name);
        const to = path.join(destination, entry.name === 'gitignore' ? '.gitignore' : entry.name);
        if (entry.isDirectory()) {
            copyTemplate(path.join(templateId, entry.name), to, projectName);
        } else {
...
async function createProject() {
    ...
    const picked = await vscode.window.showQuickPick(PROJECT_TEMPLATES, {
        title: message('promptTitle'),
    ...
    const name = await vscode.window.showInputBox({
        ...
        prompt: picked.label,
        placeHolder: message('placeholder'),
        validateInput: (value) => isValidProjectName(value) ? undefined : message('invalidName'),
    });
    if (!name) return;
    const destination = path.resolve(folder.uri.fsPath, name);
    const relative = path.relative(path.resolve(folder.uri.fsPath), destination);
    if (!isValidProjectName(name) || relative.startsWith('..') || path.isAbsolute(relative)) {
        await vscode.window.showErrorMessage(message('invalidName'));
        return;
    }
    if (fs.existsSync(destination)) {
        await vscode.window.showErrorMessage(message('exists'));
        return;
    }
    const packageName = name.toLowerCase().replace(/[^a-z0-9_]/g, '_');
    const projectName = /^[a-z_]/.test(packageName) ? packageName : `project_${packageName}`;
    try {
        copyTemplate(picked.id, destination, projectName);
        const readme = ['README.md', 'index.js', 'main.py', 'bin/main.dart']
            .map((file) => path.join(destination, file))
            .find((file) => fs.existsSync(file));
        if (readme) {
            await vscode.commands.executeCommand('vscode.open', vscode.Uri.file(readme));
        }
        await vscode.window.showInformationMessage(message('created').replace('{0}', name));
    } catch (error) {
        await vscode.window.showErrorMessage(String(error && error.message ? error.message : error));
    }
}
...
function activate(context) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('vscodroid.createProject', createProject)
    );
...
module.exports = { activate, deactivate, createProject, isValidProjectName, PROJECT_TEMPLATES };
