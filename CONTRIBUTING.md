...
| `test-input-preview.js` | Runs the landscape preview script against EditContext, input, focus, and fallback-input events, and checks that deletion clears the displayed text without reinstalling the listeners | exit status |
| `test-local-runner.js` | Exercises local task discovery, package scripts, Makefile targets, shell quoting, command registration, and the no-workspace guard | exit status |
